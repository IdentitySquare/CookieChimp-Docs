...
An account API key identifies exactly one account, so there is nothing to choose. A user API key can reach several, and account-scoped endpoints accept an `account_id` query parameter — the account's API id — to say which one you mean.

For reads it is optional and narrows the request. For writes it is required whenever the key can reach more than one account: without it the request would act on an account it picked rather than one you named, so it is refused with a `400`.
