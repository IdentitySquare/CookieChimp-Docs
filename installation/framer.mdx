...
<AccordionGroup>
  ...
  <Accordion title="Will CookieChimp slow down my Framer site?">
    No. The CookieChimp script is under 30KB gzipped, loads asynchronously, and is served from a global CDN. It doesn't block rendering and won't affect Framer's Lighthouse scores in any meaningful way.
  </Accordion>

  <Accordion title="Does CookieChimp support Google Consent Mode v2 on Framer?">
    Yes. CookieChimp pushes the correct `gtag('consent', ...)` calls automatically. See [Google Consent Mode](/integrations/google-consent-mode) for the full setup, which works the same on Framer as on any other site.
  </Accordion>

  <Accordion title="Can I customize the cookie banner to match my Framer design?">
    Yes. The banner is fully themable from the [Banner Designer](/features/banner-designer) — colors, typography, corner radius, button styles and copy can all be matched to your Framer brand.
  </Accordion>

  <Accordion title="Is CookieChimp free for Framer sites?">
    CookieChimp has a free tier suitable for small Framer sites, with paid plans for higher traffic and additional features. See [pricing](https://cookiechimp.com/pricing).
  </Accordion>
</AccordionGroup>
