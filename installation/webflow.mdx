...
<AccordionGroup>
  ...
  <Accordion title="Will CookieChimp affect my Webflow site's PageSpeed score?">
    No. The CookieChimp script is under 30KB gzipped, loads asynchronously, and is served from a global CDN. Webflow's Lighthouse scores stay essentially unchanged.
  </Accordion>

  <Accordion title="Does CookieChimp support Google Consent Mode v2 on Webflow?">
    Yes. CookieChimp pushes the correct `gtag('consent', ...)` calls automatically, including the new `ad_user_data` and `ad_personalization` signals required by Consent Mode v2. See [Google Consent Mode](/integrations/google-consent-mode).
  </Accordion>

  <Accordion title="Can I customize the cookie banner to match my Webflow brand?">
    Yes. The banner is fully themable from the [Banner Designer](/features/banner-designer) — colors, typography, corner radius, button styles and copy can all be matched to your Webflow style guide.
  </Accordion>
