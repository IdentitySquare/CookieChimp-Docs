...
<Steps>
  ...
  <Step title="Add/Edit Tag">
    ...
    Create or update a tag and add an additional consent check so the tag only fires once the matching vendor has consent.
      ...
      Navigate to "Advanced Settings" > "Consent Settings". Select "Require additional consent for tag to fire" and enter the value you want to gate the tag on.
      ...
      **Recommended: gate the tag on the vendor name.**

      Enter the exact vendor name as it appears in your CookieChimp [Vendor Library](/features/vendor-management) — for example, `Google Analytics`, `Hotjar`, or `Facebook Pixel`. CookieChimp sends the consent state of each vendor to Google, so the tag will only fire when that specific vendor has been granted consent in the banner.
      ...
      **Alternative: gate the tag at the category level.**
